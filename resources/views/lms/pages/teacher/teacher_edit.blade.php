...
<div class="card">
  ...
  <div class="">
    <h4>Edit Teacher ({{$teacher->user->firstname}},{{$teacher->user->lastname}})</h4>
      ...
      <form method="post" action="{{route('teacher.update',$teacher->id)}}">
        ...
        <div class="row">
            ...
            <div class="form-group col-md-3 col-xs-12">
                ...
                <input type="text" class="form-control @error('mobile') is-invalid @enderror"  name="mobile" value="{{$teacher->user->mobile}}{{old('mobile')}}">
                @error('mobile')
                    <span class="" role="alert">
                        <strong><i>{{ $message }}</i></strong>
                    </span>
